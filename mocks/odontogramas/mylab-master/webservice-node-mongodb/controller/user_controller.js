...
exports.list = function(callback){
	...
	db.User.find({}, function (error, users) {
		if(error) {
			callback({error: 'Não foi possível retornar os usuarios'});
		} else {
			callback(users);
		}
	});
...
exports.user = function(id, callback) {
	...
	db.User.findById(id, function (error, user) {
		if(error) {
			callback({error: 'Não foi possível retornar o usuario'});
		} else {
			callback(user);
		}
	});
...
exports.save = function(fullname, email, password, callback) {
	new db.User({
		...
		if(error) {
			callback({error: 'Não foi possível salvar o usuario'});
		} else {
			callback(user);
		}
	});
